...
def invia_email_licenza(email: str, license_key: str, versione: str):
    ...
    if not RESEND_API_KEY:
        print(f"[EMAIL] Resend non configurato — key: {license_key}")
        return
    ...
    try:
        ...
        payload = _json.dumps({
            "from": FROM_EMAIL,
            "to": [email],
            "subject": f"La tua License Key D.O.O.M {versione.upper()}",
            "html": corpo_html
        }).encode()
...
# ── GENERA KEY MANUALE (admin) ────────────────────────────────────────────────
@app.route("/api/admin/genera", methods=["POST"])
def admin_genera():
    """Genera una license key manualmente per test o supporto."""
    data = request.get_json()
    pwd  = data.get("pwd", "")
    ...
    email    = data.get("email", "")
    versione = data.get("versione", "lite")
    ...
    key = genera_license_key(versione)
    ora = datetime.datetime.utcnow().isoformat()
    scadenza = (datetime.datetime.utcnow() + datetime.timedelta(days=31)).isoformat()
    ...
    try:
        ...
        db.commit()
        invia_email_licenza(email, key, versione)
        print(f"[ADMIN] Generata key {versione}: {key} → {email}")
        return jsonify({"key": key, "email": email, "versione": versione})
    finally:
        db.close()
